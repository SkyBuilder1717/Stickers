...
public class StickersScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(
            Button.builder(Component.translatable("btn.stickers.new"), _ -> {
                ...
                s.x = 0.4f; s.y = 0.4f;
                Minecraft.getInstance().gui.setScreen(new StickerEditScreen(this, s, true));
        ...
        dragMode = this.addRenderableWidget(
                Button.builder(Component.translatable("btn.stickers.move_mode"), _ ->
                        Minecraft.getInstance().gui.setScreen(new StickerDragScreen())
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor ctx, int mouseX, int mouseY, float a) {
        ...
        for (int i = scrollOffset; i < endIdx; i++) {
            ...
            int previewColor = opaqueVersion(s.bgColor);
            int swatchX = listX + 4;
            int swatchY = itemY + (ITEM_H - LIST_SWATCH_SIZE) / 2;
            ctx.fill(swatchX, swatchY, swatchX + LIST_SWATCH_SIZE, swatchY + LIST_SWATCH_SIZE, previewColor);
            drawBorder(ctx, swatchX, swatchY, LIST_SWATCH_SIZE, LIST_SWATCH_SIZE, 0xFF666666);

            int visColor = s.visible ? 0xFF55FF55 : 0xFF888888;
            ctx.text(this.font, s.visible ? "+" : "-", listX + 23, itemY + (ITEM_H - 8) / 2, visColor);
            ...
            ctx.text(
                    this.font,
                    Component.literal(sizeStr),
                    listX + LIST_W - sizeW - 6,
                    itemY + (ITEM_H - 8) / 2,
                    0xFFAAAAAA
            );
        }
        ...
        if (stickers.isEmpty()) {
            ctx.centeredText(this.font,
                    Component.translatable("screen.stickers.empty"),
                    cx, listY + listH / 2 - 4, 0xFF888888);
        }
    ...
    public boolean mouseScrolled(double mx, double my, double hAmt, double vAmt) {
        ...
        if (mx >= listX && mx < listX + LIST_W && my >= listY && my < listY + listH) {
            ...
            scrollOffset = Math.clamp(maxScroll, 0, scrollOffset - (int) vAmt);
            return true;
        }
        return false;
    }
    ...
    private void openEdit() {
        ...
        if (selectedIndex >= 0 && selectedIndex < list.size()) {
            ...
            Minecraft.getInstance().gui.setScreen(new StickerEditScreen(this, copy, false));
        }
    }
    ...
    private void toggleVisible() {
        List<StickerData> list = StickerManager.getAll();
        if (selectedIndex >= 0 && selectedIndex < list.size()) {
            StickerData s = list.get(selectedIndex);
            s.visible = !s.visible;
            StickerManager.save();
        }
    }
    ...
    private void refreshButtons() {
        List<StickerData> stickers = StickerManager.getAll();
        boolean has = selectedIndex >= 0 && selectedIndex < stickers.size();
        if (editBtn != null) editBtn.active = has;
        if (deleteBtn != null) deleteBtn.active = has;
        if (toggleBtn != null) toggleBtn.active = has;
        if (dragMode != null) dragMode.active = !stickers.isEmpty();
    }

    private static int opaqueVersion(int argb) {
