...
public class StickerEditScreen extends Screen {
    ...
    public StickerEditScreen(StickersScreen parent, StickerData sticker, boolean isNew) {
    ...
    protected void init() {
        ...
        int fieldH = rightH;
        textField = MultiLineEditBox.builder().build(this.font, leftW, fieldH, Component.translatable("screen.stickers.edit"));
        ...
        addRight(borderTypeBtn = Button.builder(
                ...
                b -> {
                    ...
                    b.setMessage(Component.translatable(sticker.borderOuter ? "btn.stickers.border_outer" : "btn.stickers.border_inner"));
    ...
    @Override
    public void extractBackground(@NonNull GuiGraphicsExtractor ctx, int mouseX, int mouseY, float a) {
        super.extractBackground(ctx, mouseX, mouseY, a);
    ...
    @Override
    public void extractRenderState(@NonNull GuiGraphicsExtractor ctx, int mouseX, int mouseY, float a) {
        ...
        ctx.text(this.font,
                Component.translatable("section.stickers.colors"), rightX, colorLabelY, 0xFFE0E0E0);

        renderColorHeader(ctx, Component.translatable("label.stickers.bgcolor"),
                rightX,           bgHdrY, sticker.bgColor);
        renderColorHeader(ctx, Component.translatable("label.stickers.txtcolor"),
                rightX + hw + CG, bgHdrY, sticker.textColor);

        renderColorHeader(ctx, Component.translatable("label.stickers.bordercolor"),
                rightX, borderLabelY, sticker.borderColor);
    ...
    @Override public void removed() {
        StickerManager.previewSticker = null;
    }
    ...
    private void cancel() { Minecraft.getInstance().gui.setScreen(parent); }
    ...
    private <T extends AbstractWidget> void addRight(T w) {
    ...
    private void renderColorHeader(GuiGraphicsExtractor ctx, Component label, int x, int y, int color) {
        ...
        ctx.text(this.font, label, x + 16, y + 2, 0xFFCFCFCF);
    }
    ...
    private static class IntSlider extends AbstractSliderButton {
        ...
        IntSlider(int x, int y, int w, int h, String label, String subfix, int min, int max,
                  int init, Consumer<Integer> onChange) {
            super(x, y, w, h, Component.literal(label + ": " + init + subfix),
                    (double)(init - min) / (max - min));
            this.label = label;
            this.subfix = subfix;
            this.min = min;
            this.max = max;
        ...
        @Override protected void updateMessage() { setMessage(Component.literal(label + ": " + getVal() + subfix)); }
