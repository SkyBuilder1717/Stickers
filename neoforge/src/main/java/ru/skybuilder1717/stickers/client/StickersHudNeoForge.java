...
public class StickersHudNeoForge {
    public static void render(GuiGraphicsExtractor ctx, DeltaTracker ticker) {
        ...
        String previewId = StickerManager.previewSticker != null
                ? StickerManager.previewSticker.id : null;
    ...
    private static void drawText(GuiGraphicsExtractor ctx, Minecraft mc, StickerData s) {
        ...
        for (String line : lines) {
            int tw = font.width(line) * s.fontScale;
            ...
            if (s.fontScale != 1) {
                ...
                ctx.text(font, line, 0, 0, s.textColor, false);
                ctx.pose().set(savedInner);
            } else {
                ctx.text(font, line, startX, startY, s.textColor, false);
            }
            startY += lineH;
        }
    }
